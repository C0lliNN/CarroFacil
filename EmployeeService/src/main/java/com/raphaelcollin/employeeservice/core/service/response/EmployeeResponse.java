...
public record EmployeeResponse(String id, String name, String email, String token) {
    public static EmployeeResponse fromEmployee(Employee employee, User user) {
        return new EmployeeResponse(employee.getId(), employee.getName(), user.getEmail(), user.getToken());
    }
}
